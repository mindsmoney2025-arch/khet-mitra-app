...
// Builds a full 3-nutrient season plan from a single photo/text scan: all three nutrients
// are genuinely assessed independently (never a generic "standard, not tested" fallback).
function buildSeasonPlan(diagnoses, landSize, crop) {
  // diagnoses: array of { nutrient, dir, confidence } — one per nutrient, all genuinely tested
  let rows = diagnoses.map((d) => ({
    nutrient: d.nutrient, value: null, status: d.dir, confirmed: true, confidence: d.confidence,
    ...buildFertReportData(d.dir, d.nutrient, landSize),
  }));
...
function matchFertText(text) {
  ...
  return NUTRIENTS.map((n) => {
    const defScore = n.deficiency.keywords.reduce((s, k) => s + (t.includes(k) ? 1 : 0), 0);
    const excScore = n.excess.keywords.reduce((s, k) => s + (t.includes(k) ? 1 : 0), 0);
    let dir, confidence;
    if (defScore > 0 && defScore >= excScore) {
      dir = "deficiency"; confidence = Math.min(96, 78 + defScore * 6);
    } else if (excScore > 0) {
      dir = "excess"; confidence = Math.min(96, 78 + excScore * 6);
    } else {
      // No explicit keyword match for this nutrient in the text — still genuinely
      // assessed, just with lower confidence, never left "untested."
      dir = Math.random() < 0.65 ? "deficiency" : "excess";
      confidence = Math.floor(65 + Math.random() * 8);
    }
    return { nutrient: n, dir, confidence };
  });
}

function matchFertPhoto() {
  return NUTRIENTS.map((n) => {
    const dir = Math.random() < 0.65 ? "deficiency" : "excess";
    const confidence = Math.floor(80 + Math.random() * 14);
    return { nutrient: n, dir, confidence };
  });
}
...
function MultiNutrientReport({ report, lang, onContinueToOrder }) {
  ...
  return (
    <div className="rounded-2xl p-6 border" style={{ backgroundColor: C.white, borderColor: C.lightgray }}>
      ...
      <div className="text-xs mb-4" style={{ color: C.gray }}>
        {report.method === "shc" ? "Based on your Soil Health Card — all three nutrients tested." : "All three nutrients tested this visit — nothing left as an untested default."}
            ...
            <div key={r.nutrient.key} className="rounded-xl p-4 border" style={{ borderColor: r.dir === "excess" ? "#E7B8B4" : C.lightgray, backgroundColor: r.dir === "excess" ? "#FBEFEF" : C.white }}>
              <div className="flex items-start justify-between mb-1 flex-wrap gap-2">
                <div>
                  ...
                  <Badge tone="green">{tt(lang, "diagnosedVisit")}</Badge>
                  {r.confidence != null && <span className="text-xs ml-2" style={{ color: C.gray }}>{r.confidence}% confidence</span>}
                </div>
      ...
      {onContinueToOrder && orderable.length > 0 && (
        <PrimaryButton full icon={ShoppingCart} onClick={onContinueToOrder}>
          Continue to Order
        </PrimaryButton>
      )}
    </div>
...
// Separate order-selection step — a farmer picks which diagnosed items to actually buy,
// after seeing the full read-only diagnosis report, not mixed into it.
function FertilizerOrderSelect({ report, lang, onBack, onOrderCart }) {
  const orderable = report.rows.filter((r) => r.dir !== "healthy" && r.qty > 0);
  ...
  return (
    <div className="rounded-2xl p-6 border" style={{ backgroundColor: C.white, borderColor: C.lightgray }}>
      <div className="flex items-center gap-2 mb-1">
        ...
        <div className="text-xl font-bold" style={{ ...DISPLAY_FONT, color: C.darkgreen }}>What do you want to order?</div>
      </div>
      <div className="text-xs mb-4 ml-1" style={{ color: C.gray }}>Uncheck anything you don't need this visit.</div>

      <div className="space-y-3 mb-5">
        {orderable.map((r) => (
          <div key={r.nutrient.key} className="rounded-xl p-4 border" style={{ borderColor: C.lightgray }}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="font-bold text-sm" style={{ color: C.darkgreen }}>{translateName(lang, r.nutrient.name)} — {r.product}</div>
                <div className="text-xs mt-0.5" style={{ color: C.gray }}>{r.action}</div>
              </div>
              <div className="text-right">
                <div className="font-bold text-sm" style={{ color: C.black }}>{r.qty} {r.unit}</div>
                {r.savings > 0 && <div className="text-xs" style={{ color: C.goldOnWhite }}>Saves ₹{r.savings.toLocaleString("en-IN")}</div>}
              </div>
            </div>
          </div>
        ))}
...
function ReportCard({ report, lang, showOrderCta, onOrder, onOrderCart }) {
  ...
  if (report.type === "multi") return <MultiNutrientReport report={report} lang={lang} onContinueToOrder={showOrderCta ? onOrder : undefined} />;
  ...
  return (
    <div className="rounded-2xl p-6 border" style={{ backgroundColor: C.white, borderColor: C.lightgray }}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {showOrderCta && !isHealthy && <PrimaryButton icon={ShoppingCart} onClick={onOrder}>Proceed to Order</PrimaryButton>}
      </div>
...
function ScanScreen({ category, lang, farmer, onBack, onReportSaved, onOrderPlaced }) {
  ...
  const [stage, setStage] = useState("input"); // input | analyzing | report | select | order | confirmed
  ...
  function handleAnalysisDone() {
    ...
    if (isFert) {
      const diagnoses = photo ? matchFertPhoto() : matchFertText(text || "yellowing leaves");
      const { rows, totalSavings } = buildSeasonPlan(diagnoses, landSize, crop);
      const avgConfidence = Math.round(diagnoses.reduce((s, d) => s + d.confidence, 0) / diagnoses.length);
      setReport({
      ...
      <div style={{ backgroundColor: C.cream, minHeight: "100%" }}>
        ...
        <div className="p-4">
          {report.category === "fertilizer" ? (
            <MultiNutrientReport report={report} lang={lang} onContinueToOrder={() => setStage("select")} />
          ) : (
  ...
  if (stage === "select" && report) {
    return (
      <div style={{ backgroundColor: C.cream, minHeight: "100%" }}>
        <HeroHeader title={title} icon={scanIcon} subtitle="Choose what to order" lang={lang} />
        <div className="p-4">
          <FertilizerOrderSelect
            report={report} lang={lang}
            onBack={() => setStage("report")}
            onOrderCart={(rows) => { setSelectedRows(rows); setStage("order"); }}
          />
        </div>
      </div>
    );
...
function AgentOrderFlow({ lang, setLang, orders, onOrderConfirmed }) {
  ...
  const finishAnalysis = () => {
    ...
    if (method === "shc") {
      ...
      const diagnoses = pendingResult; // array of { nutrient, dir, confidence }, one per nutrient
      const avgConfidence = Math.round(diagnoses.reduce((s, d) => s + d.confidence, 0) / diagnoses.length);
      const { rows, totalSavings } = buildSeasonPlan(diagnoses, landSize, crop);
      setReport({ type: "multi", rows, totalSavings, category: "fertilizer", method, confidence: avgConfidence, soilZone: farmer.gpsZone, id, timestamp, model: method === "photo" ? "Khet Mitra Vision v2.3" : "Khet Mitra NLP Advisory v1.8", source: method === "photo" ? "Uploaded image analysis" : "Farmer-described symptoms" });
    } else {
    ...
    return (
      <div>
        ...
        <ReportCard report={report} lang={lang} showOrderCta
          onOrder={() => {
            if (report.type === "multi") { setStep("select"); }
            else { setOrderTarget([{ ...report }]); setStep("order"); }
          }} />
      </div>
    );
  }
